...
/*  Created On : 28-Nov-2016 11:00:48 PM 				*/
/*  DBMS       : MySql 						*/
/* ---------------------------------------------------- */
...
SELECT uuid_short() as id, club_service_id, end_bill_date as date, CONCAT(DATE_FORMAT(end_bill_date, '%M, %Y'), ' Bill') as description, bill_amount as amount, amount_due as balance
FROM gn_club_bill
UNION ALL
SELECT uuid_short() as id, club_service_id, payment_date as date, (CASE WHEN payment_amount >= 0 THEN 'Payment to Association' ELSE 'Payment to Club' END) as description, (payment_amount * -1) as amount, null as balance
FROM gn_club_payment
UNION ALL
SELECT uuid_short() as id, club_service_id, adj_date as date, adj_description as description, adj_amount as amount, null as balance
