 ...
 ********************************************************************************/
	function mf_process_form($dbh,$input){
		...
		if(($is_inserted && !empty($logic_email_enable) && empty($form_review) && ($form_page_total == 1) && empty($edit_id) ) || 
		   ($is_inserted && !empty($logic_email_enable) && $is_committed && empty($edit_id) )
		){
		...
		if(($is_inserted && !empty($logic_webhook_enable) && empty($form_review) && ($form_page_total == 1) && empty($edit_id) ) || 
		   ($is_inserted && !empty($logic_webhook_enable) && $is_committed && empty($edit_id) )
		){
	...
	function mf_commit_form_review($dbh,$form_id,$record_id,$options=array()){
		...
		$query = "UPDATE `ap_form_{$form_id}` SET date_created = ? WHERE `id` = ?";
