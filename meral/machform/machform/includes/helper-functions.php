...
EOT;
    ...
    function mf_send_logic_webhook_notifications($dbh,$form_id,$entry_id){
    ...
    function mf_process_delayed_notifications($dbh,$form_id,$entry_id,$options=array()){
		...
		if(!empty($logic_webhook_enable)){
			mf_send_logic_webhook_notifications($dbh,$form_id,$entry_id);
		}
    }
			...
			foreach ($entry_details as $data){
				...
				if($data['element_type'] == 'textarea'){
					...
					if(!empty($email_param['machform_base_path'])){ //if the form is called from advanced form code
						$data['value'] = '<img src="'.$email_param['machform_base_path'].'signature_img.php?q='.$q_string.'" />';
					}else{
