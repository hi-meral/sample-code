...
class Spreadsheet_Excel_Reader {
	...
	// DUMP AN PHP Array OF THE ENTIRE XLS DATA
	// =========================================
	
	function dumparray_old($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
                $out =array();
		$countershow=0;
		//$out[] = "<table class=\"$table_class\" cellspacing=0>";
		if ($col_letters) {
			...
			for($i=1;$i<=$this->colcount($sheet);$i++) {
				...
				$out[$countershow][$i] = strtoupper($this->colindexes[$i]) ;
			}			
		}
	...
	function dumparray($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
                $out =array();
		$countershow=0;
		if ($col_letters) {
			//$out[] = "<thead>\n\t<tr>";
